...
class RuleMaJiang(RuleBase):
    ...
    @staticmethod
    def is_seven_pairs(cards, nai_zi=ma_jiang.NAI_ZI):
        """判断是否7小对"""
        if not cards or len(cards) != 14:
            return False, []

        cards = list(cards)
        nai_zi_count = remove_by_value(cards, ma_jiang.NAI_ZI, -1)
        count_list = RuleMaJiang.get_count_list_by_value(cards)
        ...
        if singles_len <= 0:
            tmp =  [[card] * count for card, count in count_list.items()]
            if nai_zi_count != 0:
                tmp.append([ma_jiang.NAI_ZI] * nai_zi_count)
            return True, tmp
