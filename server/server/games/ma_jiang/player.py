...
from games.ma_jiang import ma_jiang, flow
from models import logs_model
from models import onlines_model
from models import players_model, floor_model, club_model, union_model, database
...
class Player(object):
    ...
    def session(self, session):
        if self.__session and session != self.__session:
            self.clear_session()
    ...
    def can_hu(self, card, rule, is_seven_pairs, check_lou_hu=True):
        ...
        return rule.can_hu(self.__zhuo_pai, self.__cards, card, is_seven_pairs)
