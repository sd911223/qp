...
class ZzMaJiangJudge(BaseJudge):
    def __init__(self, ret, service_type):
        ...
        self.__limit_score = int(self.rule_details.get("", 0))
    ...
    def __fetch_rule(self):
        if ma_jiang.ZZ_MA_JIANG == self.rule_type:
            from games.ma_jiang.rule_zz_ma_jiang import RuleZZMaJiang
    ...
    def __calc_check_out_result(self, winner_list):
        ...
        for p in winner_list:
            ...
            qiang_gang_hu_score = bool(self.__hong_zhong) and 6 or 1
            is_qiang_gang_hu = self.flow_status == flow.T_IN_MING_GANG_PAI_CALL
            score = 0
            if not is_qiang_gang_hu:
                if is_zi_mo:
                    score = 2
                else:
                    score = 1
            ...
            for data in p.zhuo_pai:
                if data[0] in (
                        ma_jiang.ACTION_TYPE_MING_GANG, ma_jiang.ACTION_TYPE_GONG_GANG,
                        ma_jiang.ACTION_TYPE_AN_GANG) and \
                        data[1] not in p.gang_peng_cards:
                    gang_count += 1

            result.append({"score": score, "is_hai_di": self.__ma_jiang_cards.left_count <= 0, "is_zi_mo": is_zi_mo,
                           "zhuang_xian_score": self.__zhuang_xian and 1 or 0,
                           "qiang_gang_hu_score": is_qiang_gang_hu and qiang_gang_hu_score or 0,
                           "gang_count": gang_count, "is_qiang_gang_hu": is_qiang_gang_hu, "winner": p})
    ...
    def __dismiss_check_out(self):
        ...
        # is_gang_effect = not bool(self.__hong_zhong)
        # return [], self.deal_gang_info({}, {}, is_gang_effect)
    ...
    def __calc_win_score(self, check_out_result, bird_list, winner_list: list):
        ...
        for hu_value in check_out_result:
            ...
            if self.player_total_count() == 3 and self.__bird_type:
                ...
                pos_map = [2, 1]
            elif self.player_total_count() == 2 and self.__bird_type and self.__si_fang_niao:
                pos_map = [4, 1, 3, 2]
            ...
            for bird in bird_list:
                ...
                if self.__bird_count == 1 and self.__yi_ma_quan_zhong:  # 一码全中
                    ...
                    if pos == 1 and not self.__bird_type:
            ...
            for seat, _ in lose_info.items():
                ...
                deal_lose_info(seat, key, hu_value["score"])
                hu_score += hu_value["score"]
            ...
            if not hu_value["is_qiang_gang_hu"]:
                if hu_value["is_zi_mo"]:
                    win_score_info[winner.seat_id]["ziMo"] = hu_score * self.match_enter_score
                else:
                    win_score_info[winner.seat_id]["jiePao"] = hu_score * self.match_enter_score
    ...
    def __make_win_info(self, bird_list):
        ...
        for winner in self.__winner_list:
            _, hu_path = winner.can_hu(hu_card, self.__rule, bool(self.__seven_dui), False)
    ...
    def __round_over(self, win_seat_list, is_huang_zhuang, is_dismiss):
        ...
        for p in self.seats:
            ...
            if not p.is_auto_chupai:
                continue
            p.delay_ready = DelayCall(3,self.__delay_ready,p)
            if p.is_ready:
    ...
    def __chi_de_qi(self, p: Player, card):  # 判断玩家能否吃牌
        ...
        if self.flow_status == flow.T_IN_PUBLIC_OPRATE:
            ...
            if p == self.__prev_player(self.__curr_player()):
                return False
        _canpengdeqi =  p.can_chi(card, self.__rule)
        if _canpengdeqi:
    ...
    def __peng_de_qi(self, p: Player, card):
        if not p:
            return False
        _canpengdeqi = p.can_peng(card, self.__rule)
        if _canpengdeqi:
            self.user_time_out_operator(p)
        return _canpengdeqi
    ...
    def __hu_de_qi(self, p: Player, card: int, hu_type):
        ...
        can_hu, _ = p.can_hu(card, self.__rule, bool(self.__seven_dui))
        if can_hu:
            self.user_time_out_operator(p)
        return can_hu
    ...
    def __calc_operates_after_ming_gang(self, p: Player, card: int):
        result = []
        if not p:
            return result
    ...
    def __enter_ming_gang_call(self, curr_player, card):
        ...
        for p in self.seats:
            ...
            operates = self.__calc_operates_after_ming_gang(p, card)
    ...
    def check_action_end(self):
        ...
        action_map = {ma_jiang.ACTION_TYPE_HU: self.__somebody_hu, ma_jiang.ACTION_TYPE_PENG: self.__somebody_peng,
        ...
        for seat_id in hu_list:
            ...
            data.append(
                {"huCards": [hu_card], "seatID": p.seat_id, "preSeatID": self.__curr_player().seat_id, "isFinish": 1,
                 ...
                 "isQiangGangHu": self.flow_status == flow.T_IN_MING_GANG_PAI_CALL, "flow": self.flow_status})
    ...
    def __somebody_gong_gang(self, gang_list: list) -> bool:
        ...
        if flag:
            ...
            self.__curr_card_exist = 0
            if card not in p.gang_peng_cards:
    ...
    def player_join(self, player: Player):
        ...
        #    player.is_ready = self.status > flow.T_READY  # or self.round_index == 1  # 游戏中坐下时自动准备 TODO
    ...
    def __add_review_log(self, cmd, data):
        message = protocol_utils.pack_client_message(const.SERVICE_ZZMJ, cmd, deepcopy(data))
