...
def get_all_config_items(db):
    # 从DB里面加载全局配置
    sql = 'SELECT * FROM {0}'.format(table_name.share_activity_config)
    ret = db.query(sql)
    items = {}
    for item in ret:
...
def set_config(conn, key, value):
    ...
    sql = "UPDATE `{0}` SET `value`='{1}' WHERE `var`='{2}' LIMIT 1". \
        format(table_name.share_activity_config, value, key)
...
def set_all_configs(db, params):
    ...
    for k, v in params.items():
        sql = "UPDATE `{0}` SET value='{1}' WHERE var='{2}'".format(table_name.share_activity_config, v, k)
        try:
